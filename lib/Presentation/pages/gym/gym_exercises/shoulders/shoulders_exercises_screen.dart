...
class ShouldersExercisesScreen extends StatefulWidget {
  const ShouldersExercisesScreen({super.key});

  @override
  State<ShouldersExercisesScreen> createState() => _ShouldersExercisesScreenState();
}
...
class _ShouldersExercisesScreenState extends State<ShouldersExercisesScreen> {
  ...
  Future<void> _loadExercises() async {
    ...
    final data = await SupabaseService.getGymExercises(category: 'Shoulders', page: 0, pageSize: _pageSize);
    if (mounted) setState(() {
      _exercises = data;
      _isLoading = false;
  ...
  Future<void> _loadMore() async {
    ...
    final data = await SupabaseService.getGymExercises(category: 'Shoulders', page: _page, pageSize: _pageSize);
    if (mounted) setState(() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text('No shoulders exercises available', style: TextStyle(color: context.subtextColor, fontSize: 16)),
          ],
        ),
      );
    }
          ...
          child: GestureDetector(
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => ShouldersExerciseDetail(exercise: exercise),
                ),
              );
            },
            child: Container(
...
Text(exercise['description'] ?? '', style: const TextStyle(color: Colors.white70, fontSize: 12,
    shadows: [Shadow(color: Colors.black, blurRadius: 10)]), maxLines: 1, overflow: TextOverflow.ellipsis),                            const SizedBox(height: 8),
                            Row(
                              children: [
