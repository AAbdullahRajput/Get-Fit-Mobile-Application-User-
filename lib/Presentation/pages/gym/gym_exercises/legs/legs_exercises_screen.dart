...
class _LegsExercisesScreenState extends State<LegsExercisesScreen> {
  ...
  Future<void> _loadExercises() async {
    ...
    final data = await SupabaseService.getGymExercises(category: 'Legs', page: 0, pageSize: _pageSize);
    if (mounted) setState(() {
      _exercises = data;
      _isLoading = false;
  ...
  Future<void> _loadMore() async {
    ...
    final data = await SupabaseService.getGymExercises(category: 'Legs', page: _page, pageSize: _pageSize);
    if (mounted) setState(() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text('No legs exercises available', style: TextStyle(color: context.subtextColor, fontSize: 16)),
          ],
        ),
      );
    }
          ...
          child: GestureDetector(
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => LegsExerciseDetail(exercise: exercise),
                ),
              );
            },
            child: Container(
