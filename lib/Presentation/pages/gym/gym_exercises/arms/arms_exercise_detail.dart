...
Future<void> _checkFavorite() async {
    try {
      ...
      debugPrint('\x1B[33m[FAV] isFavorite result: $result for ${widget.exercise['id']}\x1B[0m');
      if (mounted) setState(() { _isFavorite = result; _isLoading = false; });
    } catch (e) {
      debugPrint('\x1B[31m[FAV] _checkFavorite error: $e\x1B[0m');
  ...
  Future<void> _toggleFavorite() async {
    ...
    try {
      ...
      debugPrint('\x1B[32m[FAV] toggled, now: ${!_isFavorite}\x1B[0m');
                              ...
                              Row(
                                children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                              color: _levelColor(widget.exercise['level'] ?? ''),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Wrap(
                          spacing: 8,
                          children: [
                            _muscleChip(context, 'Biceps'),
                            _muscleChip(context, 'Triceps'),
                            _muscleChip(context, 'Brachialis'),
                            _muscleChip(context, 'Forearms'),
                          ],
                        ),
                        const SizedBox(height: 20),
                        ...
                        Container(
                          ...
                          child: Text('Dumbbells', style: TextStyle(color: context.textColor, fontSize: 14)),
                        ),
                        const SizedBox(height: 20),
                        ...
                        Container(
                          ...
                          child: Column(
                            children: [
                              _instructionItem(context, '1', 'Stand with arms fully extended'),
                              _instructionItem(context, '2', 'Curl the weight up towards your shoulders'),
                              _instructionItem(context, '3', 'Squeeze your biceps at the top'),
                              _instructionItem(context, '4', 'Lower the weight back down with control'),
                            ],
                          ),
                        ),
                        const SizedBox(height: 20),
                        Row(
                          children: [
                            ...
                            Container(
                              ...
                              decoration: BoxDecoration(color: context.cardBgColor, borderRadius: BorderRadius.circular(12)),
                              child: _isLoading
                                      ? Center(child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: _accent(context))))                                  : IconButton(
                                      icon: Icon(
