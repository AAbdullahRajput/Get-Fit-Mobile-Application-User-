...
class _ArmsExerciseDetailState extends State<ArmsExerciseDetail> {
  ...
  void _parseSetsReps() {
    ...
    final totalKcal = int.tryParse(RegExp(r'\d+').firstMatch(kcalStr)?.group(0) ?? '200') ?? 200;
    _totalCalories = totalKcal;
  }
  ...
  void _markSetDone() {
    ...
    setState(() {
      _totalDurationSeconds += setDurationSecs;
      _setDone = true;
    });
  ...
  Widget _buildMuscles() {
    ...
    List<String> muscles = [];
    if (raw is List) {
      muscles = raw.map((e) => e.toString()).toList();
  ...
  Widget _buildInstructions() {
    final raw = widget.exercise['instructions'];
    List<String> steps = [];
    if (raw is List) {
      steps = raw.map((e) => e.toString()).toList();
    } else if (raw is String && raw.isNotEmpty) {
      steps = [raw];
    }
    if (steps.isEmpty) steps = ['Follow standard form for this exercise.'];
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: context.cardBgColor, borderRadius: BorderRadius.circular(12)),
      child: Column(
        children: steps.asMap().entries.map((e) =>
            _instructionItem(context, '${e.key + 1}', e.value)).toList(),
      ),
    );
  }
      ...
      child: Column(
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(_totalSets, (i) {
          ...
          const SizedBox(height: 16),
          if (_resting) ...[
            Text('Rest Time', style: TextStyle(color: context.subtextColor, fontSize: 14)),
            const SizedBox(height: 8),
            ...
            Text(_resting ? 'Rest before Set ${_currentSet + 1}' : 'Set $_currentSet of $_totalSets',
  ...
  Widget _muscleChip(BuildContext context, String muscle) => Container(
    ...
    child: Text(muscle,
        style: TextStyle(color: context.isDark ? themeColor : Colors.black87,
